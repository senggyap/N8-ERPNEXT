/* NKT CURRENT CLIENT SCRIPT — NKT Cashier Fast Screen — Form
...
/* ===== SOURCE: NKT Cashier Fast Screen V2.0C.3 ===== */
(() => {
  const MODE = 'cashier';
  ...
  function shell_markup() {
    ...
    return `
      <div class="nkt-fast-shell" tabindex="0">
        ...
        <div class="nkt-details-strip nkt-details-strip-cashier">
          <div class="nkt-details-title"><b>Receipt Details</b><span>Optional cashier note</span></div>
          <label class="nkt-detail-field nkt-detail-remarks"><span>Remarks <small>Optional</small></span><input data-role="remarks" maxlength="500" autocomplete="off" placeholder="Optional receipt or payment note"></label>
        </div>
...
function persist_transaction_details(state, result) {
  ...
  // NKT POST-C15F FC1C — Cashier Fast Customer Creation
  function open_fast_new_customer(state) {
    const d = new frappe.ui.Dialog({
      title: __('New Customer'),
      fields: [
        { fieldname: 'customer_name', fieldtype: 'Data', label: __('Customer Name'), reqd: 1, default: String(role(state, 'customer-entry').val() || '').trim() },
  ...
  function open_payment_preview(state, intent = null) {
    ...
    if (adjusted_price_rows(state).length && !state.priceAuthorization) {
      request_price_authorization(state, () => open_payment_preview(state, intent));
      return;
    }
...
/* ===== END SOURCE: NKT Cashier Fast Screen V2.0C.3 ===== */

/* ===== SOURCE: NKT R8A Cashier Frontline Presentation Guard ===== */

// NKT C15F R8A - Cashier frontline presentation guard
(() => {
    const DOCTYPE = "NKT Cashier Fast Screen";
    const TERMS = [/^\s*Request ID\b/i];
    const HIDDEN_ATTR = "data-nkt-r8a-hidden";
    ...
    function chooseTarget(el) {
        ...
        for (let i = 0; i < 5 && node; i += 1) {
            ...
            if (node.matches && node.matches("tr, p, li, .form-group, .control-group, .alert, [class*='meta']")) return node;
            if (node.parentElement && text.length < 450) node = node.parentElement;
            else break;
        }
    ...
    if (!window.__nktR8ACashierObserver && document.body) {
        window.__nktR8ACashierObserver = new MutationObserver(schedule);
        window.__nktR8ACashierObserver.observe(document.body, {childList: true, subtree: true});
    }
})();

/* ===== END SOURCE: NKT R8A Cashier Frontline Presentation Guard ===== */

/* ===== SOURCE: NKT R4 UI6 Cashier F8 Transaction History ===== */
...
/* ===== END SOURCE: NKT R4 UI6 Cashier F8 Transaction History ===== */

/* ===== SOURCE: NKT R4 UI6 Reserved F1 F7 - NKT Cashier Fast Screen ===== */
// NKT R4 UI6 - reserve F1 and F7 on this NKT Fast Screen.
(() => {
    const DOCTYPE = "NKT Cashier Fast Screen";
    const SLOT = "__nktReservedF1F7_db3fe4a04bd7cf94";
    frappe.ui.form.on(DOCTYPE, {
        refresh() {
            install();
        }
...
/* ===== END SOURCE: NKT R4 UI6 Reserved F1 F7 - NKT Cashier Fast Screen ===== */

/* ===== SOURCE: NKT R4 UI7B Cashier Payment on Account Bridge ===== */
...
/* ===== END SOURCE: NKT R4 UI7B Cashier Payment on Account Bridge ===== */
